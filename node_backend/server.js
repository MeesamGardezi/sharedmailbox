...
const oauth2Client = new google.auth.OAuth2(
    ...
    process.env.GOOGLE_REDIRECT_URI || 'https://api.mybox.buildersolve.com/auth/google/callback'
);
...
app.get('/auth/google/callback', async (req, res) => {
    ...
    try {
        ...
        const frontendUrl = process.env.FRONTEND_URL || 'https://mybox.buildersolve.com';
        res.send(`
            <html>
...
app.get('/auth/microsoft', async (req, res) => {
    ...
    console.log('[Microsoft Auth] Initializing auth provider with config:', {
        clientId: msalConfig.auth.clientId ? '${msalConfig.auth.clientId.substring(0, 5)}...' : 'MISSING',
        authority: msalConfig.auth.authority,
        redirectUri: authUrlParameters.redirectUri
    });
